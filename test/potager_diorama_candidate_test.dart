...
///
/// The diorama candidate `potager_diorama_v1.tmx` starts as a faithful copy of
/// the production `potager.tmx`. These tests prove the seam is reversible, the
/// candidate preserves the eight `emplacements de plantation` (0–7) with their
/// saved order, contacts au sol, tap behavior, the `east_upper_rock` parser
/// sentinel, and that saturated captures are deterministic at both phone sizes.
/// Later visual passes modify the candidate; these guards ensure they cannot
/// break the immutable gameplay contracts.
...
GardenSnapshot _saturatedSnapshot() {
  ...
  for (final zone in ZoneType.values) {
    final species = Species.values.where((value) => value.zone == zone).toList();
...
void main() {
  ...
  group('reversible candidate-map selection', () {
    ...
    test('production map is not modified by the candidate', () {
      final production = File('assets/maps/$_productionFile').readAsStringSync();
      final candidate = File('assets/maps/$_candidateFile').readAsStringSync();
      expect(
        production == candidate,
        isTrue,
        reason:
            'The candidate starts as a byte-identical copy of production; '
            'later visual passes modify only the candidate.',
      );
    });
  });

  group('candidate preserves structural contracts', () {
    test('candidate has the same dimensions and orientation as production',
        () async {
    ...
    test('candidate preserves the eight historical plot contacts by stable name',
        () async {
      ...
      for (final plot in plots.objects) {
        ...
        expect(plot.properties.getValue<double>('gridCol'), col, reason: plot.name);
        expect(plot.properties.getValue<double>('gridRow'), row, reason: plot.name);
        expect(adapter.fromTiledProperties(col, row), ui.Offset(x, y),
            reason: plot.name);
      }
    });
    ...
    test('candidate preserves the east_upper_rock parser sentinel', () async {
      ...
      final prodRock = (production.layerByName('props') as ObjectGroup)
          .objects
    ...
    test('candidate tile layers contain only 80x40 surface tiles', () async {
      ...
      for (final name in ['ground', 'skirt', 'path']) {
        ...
        for (final gid in layer.data!.where((gid) => gid != 0)) {
          ...
          expect((image!.width, image.height), (80, 40), reason: '$name gid $gid');
          expect(image.source!.split('/').last, matches(allowed[name]!),
              reason: '$name gid $gid');
        }
      }
    });
  ...
  group('candidate preserves runtime contacts and gameplay', () {
    ...
    test('candidate and production produce identical PotagerTiledObjects',
        () async {
      final candidate = await _loadMap(_candidateFile);
      final production = await _loadMap(_productionFile);
    ...
    test('candidate contacts are tappable through GardenGame.hitTestSlot',
        () async {
      final snapshot = _saturatedSnapshot();
      final game = GardenGame()..snapshot = snapshot;
      await game.onLoad();
  ...
  group('candidate deterministic captures', () {
    test('saturated state renders reproducibly at both phone sizes', () async {
      final snapshot = _saturatedSnapshot();
      for (final viewport in [
        (screen: '390x844', w: 390, h: 844),
        (screen: '375x667', w: 375, h: 667),
      ]) {
        final game1 = GardenGame()..snapshot = snapshot;
        await game1.onLoad();
        final bytes1 = await _renderToBytes(game1, viewport.w, viewport.h);
        game1.onRemove();

        final game2 = GardenGame()..snapshot = snapshot;
        await game2.onLoad();
    ...
    test('saturated captures differ from initial at a known pixel', () async {
      final saturated = _saturatedSnapshot();
      final initial = GardenSnapshot.initial().copyWith(
        ownedZones: ZoneType.values.toSet(),
      );

      final game1 = GardenGame()..snapshot = saturated;
      await game1.onLoad();
      final saturatedBytes = await _renderToBytes(game1, 390, 844);
      game1.onRemove();

      final game2 = GardenGame()..snapshot = initial;
      await game2.onLoad();
