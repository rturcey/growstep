...
13. En tant que développeur, je veux une empreinte de terre commune déduite de la grille pour chaque index acheté, afin que deux rendus de la même sauvegarde soient identiques.
...
Le glossaire appelle **emplacement de plantation** l'entité fonctionnelle et **empreinte de terre** sa surface graphique plate dans le Potager. Les platebandes des autres îlots conservent leur sens propre. Il réserve **décor** à un objet permanent que le joueur peut acheter, placer et déplacer. Les accessoires de cette spécification sont des **éléments d'ambiance fixes** : ils ne créent pas un nouveau décor joueur.

L'ADR-0006 autorise le rendu hybride Canvas/sprites sans changer le terrain ni les cultures. Le choix du Potager pour les emplacements est désormais une empreinte de terre Canvas ; les bacs sprites expérimentés au #49 ne sont plus utilisés. Les pas japonais et autres familles d'environnement restent régis par leurs tickets et leur validation propre.
...
4. **Couches.** Conserver peu de phases : fond, terrain, chemin et empreintes de terre plats, objets à profondeur (structures, plantes, végétation ambiante), puis signes de sélection et debug. Le classement sémantique des objets peut distinguer structure, plantation et ambiance sans imposer huit piles étanches qui annuleraient la profondeur. Dans la phase à profondeur, trier par `anchorY + zBias`, puis par priorité locale connue (ombre/plante/signe), puis par ID stable pour les égalités. Une ombre se dessine immédiatement avant son objet avec la même ancre. Les signes d'interface restent au-dessus.
...
6. **Nature des objets.** Les emplacements fonctionnels relient index sauvegardé, contact, hitbox et empreinte de terre. Les plantes proviennent seulement de `GardenSnapshot` et sont dessinées au même contact logique, au-dessus du sol. Les éléments d'ambiance fixes ont zéro callback de tap et zéro effet sur les données métier. Les décors joueur existants, s'ils sont effectivement rendus dans cette zone, gardent leur comportement propre et ne sont pas confondus avec ces éléments fixes.
7. **Zones spatiales.** Décrire au moins les emprises des huit surfaces de terre, cibles 44 × 44, chemin, structures, bordures, regroupements végétaux et une grande zone calme. Les objets fixes n'empiètent pas sur les cibles, n'occultent pas une culture et ne coupent pas le trajet. Seuls les index achetés affichent une empreinte, selon le comportement fonctionnel actuel. Les masses sont perceptives : 2 à 4 principales, 0 à 2 secondaires, au moins une zone calme ; maximum deux grandes décorations indépendantes. Ce sont des limites visuelles et non un comptage de sprites.
8. **Emplacements et terre plate.** Conserver exactement la carte d'index 0…7, les contacts historiques, la cible 44 × 44 et l'ordre 3 / 2 / 3. Chaque contact est dérivé du treillis 80 × 40 ; la même empreinte de terre Canvas de 72 × 36, inscrite dans une case 2:1, sert à tous les index achetés. Son contour organique et arrondi reste dans cette emprise, sans bordure sombre ; la matière peu contrastée laisse respirer la pelouse. Sa taille ne dépend d'aucune espèce ni de la largeur du feuillage. Tomate, carotte et courgette ont des emprises visuelles différentes : elles peuvent déborder naturellement d'une surface plate. Un cadre vertical imposerait un clipping artificiel et une ambiguïté de profondeur. La hitbox et le gameplay dépendent du contact et de l'index, jamais du contour dessiné. Les sprites de bacs restent disponibles dans le dépôt mais ne servent pas aux emplacements du Potager.
9. **Chemin.** Décrire un graphe connecté dont l'entrée reste `(195,410)`, l'axe monte en serpentant et les branches s'approchent des cultures. Encoder chaque pierre par asset parmi les six pas individuels et ancre explicite, avec intervalles irréguliers mais sans chevauchement. L'herbe reste visible entre deux pierres ; si une trace Canvas existe, elle doit être discrète et discontinue en perception. Les sprites de chemin préassemblé ne servent qu'à un besoin local justifié. Depuis le #50, `PotagerPath.routes` définit l'axe et trois branches, `PotagerPath.stones` place douze sprites sur ce réseau, et le renderer les dessine dans la phase de chemin avant les empreintes de terre. La bande Canvas continue et ses pierres dessinées ont été retirées du Potager. Les six variantes sont employées ; dix pas utilisent la taille visible 1×, deux la petite catégorie 0,8× près des cibles tactiles. Les pas plats conservent la légère ombre peinte dans leur PNG et ne reçoivent pas la seconde ombre automatique réservée aux objets en profondeur.
10. **Végétation, rochers, bordure.** Composer quelques masses asymétriques : arrière gauche plus végétal, arrière droit mixte ou vertical, un côté intermédiaire ; garder une ouverture et l'avant bas. Variantes de buissons et de rochers choisies explicitement. Touffes, herbes folles, trèfles et fleurs relient les masses ou accompagnent quelques pierres et emplacements ; aucune distribution uniforme. Les overlays de bordure sont ponctuels, conservent le contour logique et laissent au moins 40 % de la tranche avant lisible.
...
| Emplacements du Potager | Empreinte de terre Canvas commune issue de la case logique 80 × 40 ; contact et hitbox inchangés. | Huit index touchables ; surface plate secondaire ; plantes inchangées. |
...
| 2 — Emplacements | Huit contacts via la composition déclarative ; empreinte de terre plate commune, sans bac sprite. | États 4/6/8, plantes, achats, sauvegarde et huit taps intacts aux deux formats. |
...
- carte stable index → contact de grille → empreinte de terre commune et conservation des plantes et états du snapshot ;
...
Les tests existants de scène, de chemin, des huit contacts et de capture fournissent le point de départ. Conserver les douze captures reproductibles, couleur et niveaux de gris, du Potager à 390 × 844 et 375 × 667 pour 4, 6 et 8 emplacements. Le pipeline actuel capture le monde seul (390 × 450 et 375 × 380 pixels) sous des noms correspondant aux formats de téléphone. Inclure si possible une capture d'application pour vérifier la coexistence avec les commandes Flutter. Comparer avant/après et soumettre chaque image à une revue visuelle externe : lisibilité de grille, profondeur, terre plate, débordement naturel des cultures, chemin, zone calme et tranche. Une seule passe de tests verts ne vaut pas validation graphique. N'ajouter de goldens ciblés que si l'infrastructure existante les stabilise sans coût disproportionné.
...
**Visuels.** Le Potager ressemble d'abord à un jardin habité. Le terrain reste entièrement visible, avec sa tranche avant. Les surfaces de terre restent plates, identifiables et secondaires aux cultures, dont le feuillage peut légèrement dépasser. Les emplacements se lisent et se touchent, mais le motif 3 / 2 / 3 n'est pas la première impression. Le chemin entre clairement par l'avant, mène à l'arrière et laisse voir de l'herbe entre ses pierres. Quelques masses végétales périphériques asymétriques, des rochers limités et une zone calme donnent de la profondeur sans encombrer. Les cultures dominent en couleur et en gris. Les états 4, 6 et 8 tiennent aux deux formats de téléphone.
