...
`assets/sprites/` contient les PNG RGBA modulaires référencés par le manifeste et chargés par Flame. Le chemin du Potager utilise douze pas japonais du pack, choisis parmi six variantes et placés dans `PotagerPath.stones`. Le terrain Canvas est en cours de migration progressive vers les surface tiles (voir [ADR-0008](../docs/adr/0008-tiled-composition-visuelle.md), section « Migration du terrain »). Les anciens PNG de décor remplacés sont archivés dans `assets/sprite_archive/`, hors du bundle. Les maquettes complètes de `art-mockups/` et `test.png` sont des références visuelles ; elles ne sont jamais déclarées comme assets de l'application.
...
Dans Tiled, `gridCol` et `gridRow` sont des propriétés `float` en cellules entières ou demi-cellules ; `shadow` peut être `none`, `small`, `medium`, `large` ou `elongated`. Le manifeste fournit l'ombre par défaut et l'ancre canonique au runtime. La map applicative garde ses couches `ground` et `skirt` vides jusqu'à la migration de terrain prévue par ADR-0008 ; la review map exerce le kit complet hors de l'application.
