...
Les six `commun_sol_pas_pierre_tile_00.png`…`_05.png` sont des dérivés Tiled des masters `commun_sol_pas_pierre_statique_ordinaire_*.png`, pas de nouvelles sources artistiques. `python3 scripts/build_tiled_stone_tiles.py` les régénère en 80×40 transparent à partir des PNG et des ancres du manifeste. Ils ne reçoivent ni fiche YAML, ni ORA, ni exports 2×/3×, ni entrée dans le manifeste runtime. `scripts/validate_sprites.py` vérifie ces six fichiers par comparaison des pixels avec une régénération ; tout autre nouveau PNG garde le contrat complet ci-dessus.
...
Les 23 PNG végétaux hérités ont été normalisés en masters 4× avec exports 2× et 3×. Leurs originaux sont conservés dans `assets/sprite_archive/original_plants/` pour comparer les silhouettes. Le script `scripts/migrate_legacy_plants.py` documente cette migration ponctuelle. Chaque source ORA créée à partir d'un ancien PNG ou d'une génération porte une seule couche aplatie identifiée comme provisoire : elle ne recrée pas les couches de travail d'origine. `legacy_allowlist.json` est vide ; tout nouveau PNG source sans fiche échoue au validateur, hormis les six dérivés Tiled décrits ci-dessus et vérifiés séparément. Le manifeste recense les 71 sprites actifs. Les prompts et la provenance des nouveaux décors figurent dans la [revue de l'issue 41](visual-review/issue-41/implementation/README.md).
